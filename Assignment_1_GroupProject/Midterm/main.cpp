...
#include <ostream>
#include <fstream>
#include <vector> // For file read and tokenizer fucntion
// My Headers
...
int main()
{
	...
	do
	{
		...
		case 'Z':
		{
			string filename = inputString("\n\tEnter a filename for the file: ", false);
			// Output to file
...
void dataInsert(Statistics<int>& stats)
{
	...
	while (true)
	{
		...
		case 'B':
		{
			...
			{
				randNumb = MIN + rand() / (RAND_MAX / (MAX - MIN + 1) + 1);
